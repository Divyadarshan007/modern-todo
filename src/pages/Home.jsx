            ...
            <section className="min-h-screen bg-gradient-to-b from-[#0f0b1a] to-[#1a1630]  text-slate-100 pt-36">
                <div className="mx-auto max-w-screen-xl px-4 lg:px-12">
                    <div className="flex items-start gap-6">
                        <aside
                            className="w-80 bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.03)] rounded-2xl p-6 shadow-lg relative">
                            ...
                            <div className="bg-[rgba(255,255,255,0.02)] rounded-2xl p-6 shadow-lg">
                                ...
                                <div className="bg-[rgba(255,255,255,0.02)] p-4 rounded-xl mb-6">
                                    <form className="flex gap-4" onSubmit={handleSubmit}>
                                        <input type="text" id="task" value={input?.task} onChange={(e) => setInput({
                                            ...input,
                                            [e.target.id]: e.target.value
                                        })}
                                            placeholder="Enter task"
                                            className="flex-1 rounded-lg px-4 py-2 bg-transparent border border-[rgba(255,255,255,0.04)]
                            focus:outline-none"
                                        />
                                        <select id="priority" value={input?.priority} onChange={(e) => setInput({
                                            ...input,
                                            [e.target.id]: e.target.value
                                        })}
                                        ...
                                        <button
                                            className="rounded-lg px-6 py-2 bg-gradient-to-r from-[#3b9dff] to-[#304ffe] font-semibold">Add</button>
                                    </form>
                                </div>
                                                ...
                                                <div
                                                    ...
                                                    <div className="flex flex-col items-end gap-2">
                                                        <div className="flex items-center gap-2">
                                                            <button className="text-sm px-3 py-1 rounded-md bg-[rgba(255,255,255,0.03)]"
                                                                onClick={() => setEditId(task.id)}>Edit</button>
                                                            <button className="text-sm px-3 py-1 rounded-md bg-[rgba(255,255,255,0.03)]"
                                                                onClick={() => {
                        ...
                        <aside className="w-96 bg-[rgba(255,255,255,0.03)] rounded-2xl p-6 shadow-lg">
                            ...
                            <div className="space-y-4 overflow-y-auto max-h-[35vh] scrollbar ">
                                {
                                    tasks.map((task) => {
                                        return task.status == "completed" ? <div key={task.id} className="space-y-4 pb-5 ">
                                            <div
                                                className="p-4 rounded-lg border border-[rgba(255,255,255,0.03)] bg-gradient-to-br from-[rgba(255,255,255,0.01)] to-transparent flex items-start justify-between">
                                                <div className="flex-1">
                                                    <div className="flex items-center gap-3">
